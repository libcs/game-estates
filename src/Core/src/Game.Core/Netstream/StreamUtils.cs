...
namespace Game.Core.Netstream
{
    ...
    {
        public static T ToGame<T>(Uri uri, Func<object> func, out StreamSink streamSink, out string[] filePaths, string estateKey, Func<string, T, string[]> fileManager)
        {
            ...
            if (uri.IsFile)
            {
                ...
                streamSink = StreamSink.DefaultStreamSink;
            ...
            else if (string.IsNullOrEmpty(uri.Host) || uri.Host == "serv")
            {
                ...
                streamSink = uri.Host != "serv" ? StreamSink.DefaultStreamSink : new StreamSinkServer(func);
                filePaths = fileManager(path, game) ?? throw new InvalidOperationException($"{game} not available");
            }
            else
            {
                streamSink = new StreamSinkClient(uri, platform, estateKey);
