...
namespace Game.Format.Wavefront
{
    public partial class WavefrontObjectWriter : CryObjectWriter
    {
        ...
        public override void Write(string outputDir = null, bool preservePath = true)
        {
            ...
            using (var file = new StreamWriter(OutputFile_Model.FullName))
            {
                ...
                foreach (ChunkNode node in CryFile.NodeMap.Values)
                {
                    ...
                    // Don't render stream
                    if (SkipStreamNodes && node.Name.StartsWith("stream"))
                    {
                        Log($"Skipped stream node {node.Name}");
                        continue;
                    }
                ...
                foreach (ChunkCompiledPhysicalProxies tmpStream in CryFile.Chunks.Where(a => a.ChunkType == ChunkTypeEnum.CompiledPhysicalProxies))
                    WriteObjHitBox(file, tmpStream); // TODO: align these properly
            }
        }
        ...
        public void WriteObjHitBox(StreamWriter w, ChunkCompiledPhysicalProxies chunkProx)  // Pass a bone stream to write to the stream.  For .chr files (armatures)
        {
