...
namespace Niflib
{
	...
	public class NiRangeLODData : NiLODData
	{
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			{
				Nif.NifStream(out lodLevels[i3].nearExtent, s, info);
				Nif.NifStream(out lodLevels[i3].farExtent, s, info);
				if (info.version <= 0x03010000)
				{
					for (var i5 = 0; i5 < 3; i5++)
					{
						Nif.NifStream(out lodLevels[i3].unknownInts[i5], s, info);
					}
				}
			}
		}
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < lodLevels.Count; i3++)
			{
				Nif.NifStream(lodLevels[i3].nearExtent, s, info);
				Nif.NifStream(lodLevels[i3].farExtent, s, info);
				if (info.version <= 0x03010000)
				{
					for (var i5 = 0; i5 < 3; i5++)
					{
						Nif.NifStream(lodLevels[i3].unknownInts[i5], s, info);
					}
				}
			}
		}
