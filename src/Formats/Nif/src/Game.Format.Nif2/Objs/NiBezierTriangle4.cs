...
namespace Niflib
{
	...
	public class NiBezierTriangle4 : NiObject
	{
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < 4; i3++)
			{
				Nif.NifStream(unknown3[i3], s, info);
			}
			Nif.NifStream(unknown4, s, info);
			Nif.NifStream(unknown5, s, info);
			for (var i3 = 0; i3 < 24; i3++)
			{
				Nif.NifStream(unknown6[i3], s, info);
			}
		}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < 4; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Unknown 3[{i3}]:  {unknown3[i3]}");
				array_output_count++;
			}
			...
			for (var i3 = 0; i3 < 24; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Unknown 6[{i3}]:  {unknown6[i3]}");
				array_output_count++;
			}
			return s.ToString();
		}
