	...
	public class NiParticleSystemController : NiTimeController
	{
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			if (info.version >= 0x04000002)
			{
				...
				for (var i4 = 0; i4 < particles.Count; i4++)
				{
					Nif.NifStream(out particles[i4].velocity, s, info);
					Nif.NifStream(out particles[i4].unknownVector, s, info);
					Nif.NifStream(out particles[i4].lifetime, s, info);
					Nif.NifStream(out particles[i4].lifespan, s, info);
					Nif.NifStream(out particles[i4].timestamp, s, info);
					Nif.NifStream(out particles[i4].unknownShort, s, info);
					Nif.NifStream(out particles[i4].vertexId, s, info);
				}
				Nif.NifStream(out block_num, s, info);
				link_stack.Add(block_num);
			}
			...
			if (info.version <= 0x03010000)
			{
				...
				for (var i4 = 0; i4 < unknownFloats2.Count; i4++)
				{
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			if (info.version >= 0x04000002)
			{
				...
				for (var i4 = 0; i4 < particles.Count; i4++)
				{
					Nif.NifStream(particles[i4].velocity, s, info);
					Nif.NifStream(particles[i4].unknownVector, s, info);
					Nif.NifStream(particles[i4].lifetime, s, info);
					Nif.NifStream(particles[i4].lifespan, s, info);
					Nif.NifStream(particles[i4].timestamp, s, info);
					Nif.NifStream(particles[i4].unknownShort, s, info);
					Nif.NifStream(particles[i4].vertexId, s, info);
				}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < particles.Count; i3++)
			{
				...
				s.AppendLine($"        Velocity:  {particles[i3].velocity}");
				s.AppendLine($"        Unknown Vector:  {particles[i3].unknownVector}");
				s.AppendLine($"        Lifetime:  {particles[i3].lifetime}");
				s.AppendLine($"        Lifespan:  {particles[i3].lifespan}");
				s.AppendLine($"        Timestamp:  {particles[i3].timestamp}");
				s.AppendLine($"        Unknown Short:  {particles[i3].unknownShort}");
				s.AppendLine($"        Vertex ID:  {particles[i3].vertexId}");
			}
			...
			array_output_count = 0;
			for (var i3 = 0; i3 < unknownFloats2.Count; i3++)
			{
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
				{
					s.AppendLine("<Data Truncated. Use verbose mode to see complete listing.>");
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Unknown Floats 2[{i3}]:  {unknownFloats2[i3]}");
				array_output_count++;
			}
			return s.ToString();
		}
