...
namespace Niflib
{
	...
	public class NiPSysEmitterCtlrData : NiObject
	{
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < birthRateKeys.keys.Count; i3++)
			{
				Nif.NifStream(out birthRateKeys.keys[i3], s, info, birthRateKeys.interpolation);
			}
			Nif.NifStream(out numActiveKeys, s, info);
			activeKeys = new Key[numActiveKeys];
			for (var i3 = 0; i3 < activeKeys.Count; i3++)
			{
				Nif.NifStream(out activeKeys[i3], s, info, 1);
			}
		}
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < birthRateKeys.keys.Count; i3++)
			{
				Nif.NifStream(birthRateKeys.keys[i3], s, info, birthRateKeys.interpolation);
			}
			Nif.NifStream(numActiveKeys, s, info);
			for (var i3 = 0; i3 < activeKeys.Count; i3++)
			{
				Nif.NifStream(activeKeys[i3], s, info, 1);
			}
		}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < birthRateKeys.keys.Count; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Keys[{i3}]:  {birthRateKeys.keys[i3]}");
				array_output_count++;
			}
			...
			for (var i3 = 0; i3 < activeKeys.Count; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Active Keys[{i3}]:  {activeKeys[i3]}");
				array_output_count++;
			}
			return s.ToString();
		}
