...
namespace Niflib
{
	...
	public class bhkBallSocketConstraintChain : bhkSerializable
	{
		...
		/*!
		 * Restitution (amount of elasticity) of constraints. Added to the diagonal of the constraint matrix. A value of 0.0 can result in a division by zero with some
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < pivots.Count; i3++)
			{
				Nif.NifStream(out pivots[i3].pivotInA, s, info);
				Nif.NifStream(out pivots[i3].pivotInB, s, info);
			}
			...
			for (var i3 = 0; i3 < entitiesA.Count; i3++)
			{
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < pivots.Count; i3++)
			{
				Nif.NifStream(pivots[i3].pivotInA, s, info);
				Nif.NifStream(pivots[i3].pivotInB, s, info);
			}
			...
			for (var i3 = 0; i3 < entitiesA.Count; i3++)
			{
				WriteRef((NiObject)entitiesA[i3], s, info, link_map, missing_link_stack);
			}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < pivots.Count; i3++)
			{
				...
				s.AppendLine($"        Pivot In A:  {pivots[i3].pivotInA}");
				s.AppendLine($"        Pivot In B:  {pivots[i3].pivotInB}");
			}
			...
			for (var i3 = 0; i3 < entitiesA.Count; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Entities A[{i3}]:  {entitiesA[i3]}");
				array_output_count++;
			}
		...
		internal override void FixLinks(Dictionary<uint, NiObject> objects, List<uint> link_stack, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < entitiesA.Count; i3++)
			{
				entitiesA[i3] = FixLink<bhkRigidBody>(objects, link_stack, missing_link_stack, info);
			}
		...
		internal override List<NiObject> GetPtrs()
		{
			...
			for (var i3 = 0; i3 < entitiesA.Count; i3++)
			{
				if (entitiesA[i3] != null)
					ptrs.Add((NiObject)entitiesA[i3]);
			}
			if (entityA != null)
