...
namespace Niflib
{
	...
	public class NiMorphMeshModifier : NiMeshModifier
	{
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < elements.Count; i3++)
			{
				Nif.NifStream(out elements[i3].semantic.name, s, info);
				Nif.NifStream(out elements[i3].semantic.index, s, info);
				Nif.NifStream(out elements[i3].normalizeFlag, s, info);
			}
		}
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < elements.Count; i3++)
			{
				Nif.NifStream(elements[i3].semantic.name, s, info);
				Nif.NifStream(elements[i3].semantic.index, s, info);
				Nif.NifStream(elements[i3].normalizeFlag, s, info);
			}
		}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < elements.Count; i3++)
			{
				...
				s.AppendLine($"        Name:  {elements[i3].semantic.name}");
				s.AppendLine($"        Index:  {elements[i3].semantic.index}");
				s.AppendLine($"        Normalize Flag:  {elements[i3].normalizeFlag}");
			}
			return s.ToString();
		}
