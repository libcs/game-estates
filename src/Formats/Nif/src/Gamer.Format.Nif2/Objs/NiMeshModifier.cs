...
namespace Niflib
{
	...
	public class NiMeshModifier : NiObject
	{
		...
		internal override void Read(IStream s, List<uint> link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < submitPoints.Count; i3++)
			{
				Nif.NifStream(out submitPoints[i3], s, info);
			}
			Nif.NifStream(out numCompletePoints, s, info);
			completePoints = new SyncPoint[numCompletePoints];
			for (var i3 = 0; i3 < completePoints.Count; i3++)
			{
				Nif.NifStream(out completePoints[i3], s, info);
			}
		}
		...
		internal override void Write(OStream s, Dictionary<NiObject, uint> link_map, List<NiObject> missing_link_stack, NifInfo info)
		{
			...
			for (var i3 = 0; i3 < submitPoints.Count; i3++)
			{
				Nif.NifStream(submitPoints[i3], s, info);
			}
			Nif.NifStream(numCompletePoints, s, info);
			for (var i3 = 0; i3 < completePoints.Count; i3++)
			{
				Nif.NifStream(completePoints[i3], s, info);
			}
		}
		...
		public override string AsString(bool verbose = false)
		{
			...
			for (var i3 = 0; i3 < submitPoints.Count; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Submit Points[{i3}]:  {submitPoints[i3]}");
				array_output_count++;
			}
			...
			for (var i3 = 0; i3 < completePoints.Count; i3++)
			{
				...
				if (!verbose && (array_output_count > Nif.MAXARRAYDUMP))
					break;
				s.AppendLine($"        Complete Points[{i3}]:  {completePoints[i3]}");
				array_output_count++;
			}
			return s.ToString();
		}
