...
namespace Gamer.Format.Collada
{
    /// <summary>
    /// export to .dae format (COLLADA)
    /// </summary>
    /// <seealso cref="Gamer.Format.Collada.BaseRenderer" />
    public class COLLADA : BaseRenderer
    {
        ...
        public COLLADA(CryEngine cryEngine) : base(cryEngine) { }
        ...
        public void WriteLibrary_Geometries()
        {
            ...
            foreach (ChunkNode nodeChunk in CryData.Chunks.Where(a => a.ChunkType == ChunkTypeEnum.Node))
            {
                ...
                if (SkipProxyNodes && nodeChunk.Name.StartsWith("proxy"))
                {
                    Log($"Skipped proxy node {nodeChunk.Name}");
                    continue;
                }
