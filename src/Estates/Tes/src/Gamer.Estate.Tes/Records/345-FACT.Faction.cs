...
namespace Gamer.Estate.Tes.Records
{
    public class FACTRecord : Record
    {
        ...
        {
            public FADTField(BinaryFileReader r, int dataSize) => r.Skip(dataSize);
        }
        ...
        public struct XNAMField
        {
            ...
            public XNAMField(BinaryFileReader r, int dataSize, GameFormat format)
            {
                FormId = r.ReadInt32();
                Mod = r.ReadInt32();
                Combat = format > GameFormat.TES4 ? r.ReadInt32() : 0; // 0 - Neutral, 1 - Enemy, 2 - Ally, 3 - Friend
            }
        }
