...
namespace Game.Estate.Tes.Records
{
    public class SCPTRecord : Record
    {
        ...
        public class SCHDField
        {
            ...
            public void SCVRField(BinaryFileReader r, int dataSize) => Variables = r.ReadASCIIMultiString(dataSize);
        }
        ...
        public override bool CreateField(BinaryFileReader r, GameFormat format, string type, int dataSize)
        {
            switch (type)
            {
                ...
                case "SCVR": if (format != GameFormat.TES3) SLSDs.Last().SCVRField(r, dataSize); else SCHD.SCVRField(r, dataSize); return true;
                case "SCDA":
